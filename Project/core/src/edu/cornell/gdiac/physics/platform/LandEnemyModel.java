 ...
 */
public class LandEnemyModel extends CapsuleObstacle {
    // Physics constants
    ...
    private static final float DUDE_HSHRINK = 0.70f;
     ...
     */
    public LandEnemyModel(float width, float height) {
     ...
     * @param height	The object width in physics units
     */
    public LandEnemyModel(float x, float y, float width, float height, float mark1, float mark2) {
    ...
    private Texture walkSheet = new Texture(Gdx.files.internal("urchin_walk.png"));
    ...
    public void draw(GameCanvas canvas) {
        ...
        canvas.draw(currentFrame, Color.WHITE, origin.x, origin.y +10,(getX() + (float)Math.sin(getAngle())*getWidth()/4 - (float)Math.cos(getAngle())*(getWidth()/2 - (faceRight ? 0 : getWidth())))*drawScale.x,
