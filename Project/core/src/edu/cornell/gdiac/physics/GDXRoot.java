...
public class GDXRoot extends Game implements ScreenListener {
	...
	public boolean selectScreen = false;
	...
	public void create() {
		...
		select = new SelectMode(canvas, 1);
		...
		song = Gdx.audio.newSound(Gdx.files.internal("platform/track1.mp3"));
	...
	public void exitScreen(Screen screen, int exitCode) {
		if (screen == select){
			...
			controllers[current].reset(select.levelNum);
			...
			//loading.dispose();
			//loading = null;
		} else if (exitCode == 10000){
			if (WorldController.levelSelect == true){
				WorldController.levelSelect = false;
				selectScreen = true;
			...
			controllers[current].levelNum++;
			controllers[current].reset(controllers[current].levelNum);
			setScreen(controllers[current]);
