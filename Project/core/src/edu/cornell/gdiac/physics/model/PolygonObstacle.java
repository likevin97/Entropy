...
public class PolygonObstacle extends SimpleObstacle {
	...
	public PolygonObstacle(float[] points) {
		this(points, 0, 0);
		this.fixture.filter.categoryBits = Constants.PHYSICS_ENTITY;
		this.fixture.filter.maskBits = Constants.HAZARD_ENTITY | Constants.PLAYER_ENTITY;
	}
	 ...
	 * for drawing.  However, the pqoints in the polygon region may be rescaled
	 * later.
	 *
	 * @param points   The polygon vertices
	 */
	...
	public void setJSONSpikeAngle(int direction) {
		...
		if (direction == GROW_UP) {
			...
			this.setAngle(MathUtils.PI);
		}

		else if (direction == GROW_RIGHT) {
			this.setY(this.getY() + 1);
			this.setAngle(-1*MathUtils.PI/2);
		}

		else if (direction == GROW_LEFT) {
			this.setX(this.getX() + 1);
			this.setAngle(MathUtils.PI/2);
		}

		else {
			this.setAngle(-3*MathUtils.PI/4.0f);
		}

	}

	public void setJSONSpikeDirtAngle(int direction) {
