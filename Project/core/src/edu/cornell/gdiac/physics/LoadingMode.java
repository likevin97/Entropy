...
public class LoadingMode implements Screen, InputProcessor, ControllerListener {
	...
	private static final String BACKGROUND_FILE = "menu/start_resize.png";
	...
	private static final String OVERLAY = "menu/overlayVolume.png";
	private static final String POINTER = "menu/pointer.png";
	...
	private static int startPointerX;
	private static int settingsPointerX;

	private static int startPointerY;
	private static int settingsPointerY;
	private static int exitPointerY;

	private int pointerLevel;
	private float deltaCombo = 0;

	private static int[][] pointers = new int[3][2];


	private boolean onMain = true;

	private TextureRegion[] cutsceneArr = new TextureRegion[6];
	private Animation cutsceneAnim = new Animation<TextureRegion>(2f, cutsceneArr);

	private TextureRegion[] loadingArr = new TextureRegion[3];
	private Animation loadingAnim = new Animation<TextureRegion>(0.5f, loadingArr);
...
//		cutsceneArr[0] = JsonAssetManager.getInstance().getEntry("scene1", TextureRegion.class);
//		cutsceneArr[1] = JsonAssetManager.getInstance().getEntry("scene2", TextureRegion.class);
//		cutsceneArr[2] = JsonAssetManager.getInstance().getEntry("scene3", TextureRegion.class);
//		cutsceneArr[3] = JsonAssetManager.getInstance().getEntry("scene4", TextureRegion.class);
//		cutsceneArr[4] = JsonAssetManager.getInstance().getEntry("scene5", TextureRegion.class);
//		cutsceneArr[5] = JsonAssetManager.getInstance().getEntry("scene6", TextureRegion.class);

		cutsceneArr[0] = new TextureRegion(new Texture(Gdx.files.internal("cutscene/1.jpg")));
		cutsceneArr[1] = new TextureRegion(new Texture(Gdx.files.internal("cutscene/2.jpg")));
		cutsceneArr[2] = new TextureRegion(new Texture(Gdx.files.internal("cutscene/3.jpg")));
		cutsceneArr[3] = new TextureRegion(new Texture(Gdx.files.internal("cutscene/4.jpg")));
		cutsceneArr[4] = new TextureRegion(new Texture(Gdx.files.internal("cutscene/5.jpg")));
		cutsceneArr[5] = new TextureRegion(new Texture(Gdx.files.internal("cutscene/6.jpg")));

		loadingArr[0] = new TextureRegion(new Texture(Gdx.files.internal("cutscene/loading1.png")));
		loadingArr[1] = new TextureRegion(new Texture(Gdx.files.internal("cutscene/loading2.png")));
		loadingArr[2] = new TextureRegion(new Texture(Gdx.files.internal("cutscene/loading3.png")));
		...
		overlay = new Texture(OVERLAY);
		pointer = new Texture(POINTER);

		volumeOn = new Texture(Gdx.files.internal("menu/soundon.png"));
		volumeOff = new Texture(Gdx.files.internal("menu/soundoff.png"));
		...
		startPointerX = canvas.getCamWidth()/2 + overlay.getWidth()/4 + 60;

		startPointerY = canvas.getCamHeight()/2 + 85;
		settingsPointerY = canvas.getCamHeight()/2 - 10;
		exitPointerY = canvas.getCamHeight()/2 - 95;

		pointers[0][0] = startPointerX;
		pointers[0][1] = startPointerY;

		pointers[1][0] = startPointerX;
		pointers[1][1] = settingsPointerY;

		pointers[2][0] = startPointerX;
		pointers[2][1] = exitPointerY;
	...
	private void update(float delta) {
		...
		if (!cutscene){
			if (Gdx.input.isKeyJustPressed(Input.Keys.DOWN) || Gdx.input.isKeyJustPressed(Input.Keys.S)){
				pointerLevel = (pointerLevel + 1) % 3;

			}
			else if (Gdx.input.isKeyJustPressed(Input.Keys.UP) || Gdx.input.isKeyJustPressed(Input.Keys.W)){
				pointerLevel = (pointerLevel + 2) % 3;
	...
	Texture skipText = new Texture(Gdx.files.internal("cutscene/skip.png"));
	...
	private void draw() {
		...
		canvas.draw(cutsceneArr[0].getTexture(), Color.WHITE, 0, 0, canvas.getCamWidth(), canvas.getCamHeight());

		if (playButton == null) {
			//drawProgress(canvas);
			currentFrame = (TextureRegion)loadingAnim.getKeyFrame(stateTime, true);
			canvas.draw(currentFrame, 10, 10);
			...
			if (cutscene){
				...
				//start the cutscene.  if player presses enter, skip to the end (everything is already loaded so we're good to go)
				if (totalFrames >= 700 || Gdx.input.isKeyJustPressed(Input.Keys.SPACE)){
					cutscene = false;
				...
				currentFrame = (TextureRegion)cutsceneAnim.getKeyFrame(stateTime, false);

				canvas.draw(currentFrame, 0, 0);
				canvas.draw(skipText, 10, 10);
			...
			if (!cutscene){
				...
				canvas.draw(background, Color.WHITE, 0, 0, canvas.getCamWidth(), canvas.getCamHeight());
				canvas.draw(overlay, canvas.getCamWidth()/2 + overlay.getWidth()/4,
						canvas.getCamHeight()/2 - overlay.getHeight()/2);

				canvas.draw(logo, canvas.getCamWidth()/2 - 150,
						canvas.getCamHeight()/2 + overlay.getHeight()/4 - 20);

				if (sound){
					canvas.draw(volumeOn, pointers[1][0] + 30, pointers[1][1] - 10);
				}
				else{
					canvas.draw(volumeOff, pointers[1][0] + 30, pointers[1][1] - 23);
				}

				canvas.draw(pointer, Color.WHITE, pointers[pointerLevel][0], pointers[pointerLevel][1], 10, 15);
			}
	...
	public static float songVolume = 0.5f;
	...
	public void render(float delta) {
		if (active) {
			...
			if ((!cutscene && pointerLevel == 0 && Gdx.input.isKeyJustPressed(Input.Keys.ENTER))
					|| WorldController.levelSelect == true) {
			...
			else if (!cutscene && pointerLevel == 1 && Gdx.input.isKeyJustPressed(Input.Keys.ENTER)){
				if (sound){
					sound = false;
					GDXRoot.song.stop();
					PlatformController.EFFECT_VOLUME = 0.0f;
					songPlaying = false;
					...
					if (!songPlaying){
						GDXRoot.song.loop(0.5f);
						songPlaying = true;
					}


					PlatformController.EFFECT_VOLUME = 0.5f;

				}
			}
			else if (!cutscene && pointerLevel == 2 && Gdx.input.isKeyJustPressed(Input.Keys.ENTER)){
				Gdx.app.exit();
			}
	...
	public void resize(int width, int height) {
		...
		this.width = (int)(BAR_WIDTH_RATIO*width/2);
